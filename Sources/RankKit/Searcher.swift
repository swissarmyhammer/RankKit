// New to RankKit (plan.md §3a, §6 phase 3) -- the package's one-call facade:
...
// SearchableMetadata` index to RankKit's narrower `Searchable`/
...
private struct RetrievalEngine: Sendable {
    ...
    /// .computeCosineRanking`'s degradation, generalized to RankKit's
    /// `HybridRanker` seam.
    ///
